...
Log.prototype.elapsed = function(startTime, msg) {
	var elapsed = Date.now()-startTime;
	var time = (elapsed>100 ? _colors.Bright : _colors.Dim) + _colors.FgYellow + '('+elapsed+'ms)' + _colors.Reset; 
	msg = msg || ' ';
	this.vlog(msg + ' elapsed: ' + time)
};
Log.prototype.pushTime = function() {
	if (!this.times) this.times = [];
	this.times.push(Date.now())
};
Log.prototype.popTime = function(msg) {
	this.elapsed(this.times.pop(), msg)
};
